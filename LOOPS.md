...
Saved: 2026-07-31
Trigger: GitHub Action, on every PR touching `src/**` or `package.json` (`.github/workflows/doc-sync-loop.yml`), plus manual `workflow_dispatch`. Runs on the PR's own branch. Checks a bounded checklist (version numbers, command names, file paths, vsix version) rather than an open-ended full audit, and a separate deterministic shell step commits/pushes any fixes — the agent only edits files, never runs git itself. Requires an `ANTHROPIC_API_KEY` secret scoped to the "main" GitHub Environment. Also needs `github_token: ${{ secrets.GITHUB_TOKEN }}` passed explicitly (the action's default GitHub-App-based auth requires installing the Claude Code GitHub App, which this repo doesn't have) and `id-token: write` in job permissions. Revised after two real runs (~$6 combined) hit `error_max_turns` with unexplained permission denials under a broader, agent-driven-git design.
...
Saved: 2026-07-31
Trigger: GitHub Action, on release tag push `v*.*.*` (`.github/workflows/test-streak-loop.yml`), before npm-publish.yml's own build/test gate. Requires an `ANTHROPIC_API_KEY` secret scoped to the "main" GitHub Environment, plus `github_token: ${{ secrets.GITHUB_TOKEN }}` and `id-token: write` (same reasons as the doc-sync loop). Scoped in CI to real CLI behavior only (no GPU/Metal, no downloaded models) — not a full pull+serve+doctor run. The agent only edits files; a separate deterministic step opens the PR if anything changed.
